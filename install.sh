...
deploy_pow_endpoints() {
    ...
    if [[ -f "$SCRIPT_DIR/__ab/pow-verify.php" ]]; then
        ...
        log_success "Copied pow_tier.php"
    else
        log_error "pow_tier.php not found in $SCRIPT_DIR/__ab/"
        exit 1
    fi
...
install_modsecurity() {
    ...
    if [[ -f "$SCRIPT_DIR/modsecurity/ab_pow_ratelimit.conf" ]]; then
        ...
        for rule_id in 910001 910002 910003 910004; do
            if grep -r "id:$rule_id" /etc/modsecurity/ 2>/dev/null | grep -v ab_pow_ratelimit.conf | grep -q .; then
                log_warn "ModSecurity rule ID $rule_id already in use"
                has_conflict=1
            fi
        done
        
        if [[ $has_conflict -eq 1 ]]; then
            log_warn "Rule ID conflicts detected, using alternative ID range..."
            # Use the simple version with different IDs
            if [[ -f "$SCRIPT_DIR/modsecurity/ab_pow_ratelimit_simple.conf" ]]; then
                mkdir -p /etc/modsecurity
                cp "$SCRIPT_DIR/modsecurity/ab_pow_ratelimit_simple.conf" \
                    /etc/modsecurity/ab_pow_ratelimit.conf
                log_success "Installed alternative rate limit rules (ID range 920001-920005)"
            else
                log_warn "Alternative rules not found, skipping rate limiting"
                log_info "You can manually configure later if needed"
                return
            fi
        else
            mkdir -p /etc/modsecurity
